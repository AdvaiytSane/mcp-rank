...
class EnhancedListenerCapability(MatchingCapability):
    ...
    async def trigger_amn(self, user_input: str):
        ...
        await self.speak("On it — running the buyer workflow now.")
        ll_ctx = LifeLoggerContext(
            conversation_summary=user_input,
            recent_speech=list(self.recent_speech[-10:]),
            history_chunks=list(self.history_chunks),
            voice_profiles=dict(self.voice_profiles),
            room_state=dict(self.room_state),
        )
        logs, cost, success = await asyncio.to_thread(_amn_run_warm, ll_ctx)
        if success:
            actions = [l.chosen_action for l in logs]
            await self.speak(
...
# =============================================================================
# AMN — Agent Memory Network
# Real estate lead conversion with Markov policy learning.
...
AMN_CONFIDENCE_THRESHOLD = 0.70
AMN_MAX_STEPS = 20

AMN_TOOL_COSTS = {
    "extract_requirements": 0.02, "qualify_lead": 0.03,
    "search_listings": 0.05,      "filter_listings": 0.03,
    "rank_listings": 0.04,        "send_message": 0.02,
    "check_calendar": 0.01,       "schedule_tour": 0.03,
...
def _amn_llm(system: str, user: str, max_tokens: int = 100) -> str:
    resp = requests.post(
        OPENROUTER_URL,
        headers={"Authorization": f"Bearer {OPENROUTER_API_KEY}", "Content-Type": "application/json"},
        json={"model": "google/gemini-2.0-flash-001", "max_tokens": max_tokens,
...
def _amn_merge(base: dict, update: dict) -> dict:
    m = dict(base)
    for s, actions in update.items():
        m.setdefault(s, {})
        for a, e in actions.items():
            if a not in m[s]:
                ...
                x = m[s][a]
                n = x["count"] + e["count"]
                x["success_rate"] = (x["success_rate"]*x["count"] + e["success_rate"]*e["count"]) / n
                x["avg_cost"]     = (x["avg_cost"]*x["count"]     + e["avg_cost"]*e["count"]) / n
                x["count"] = n
                x["next_state"] = e["next_state"]
    return m

def _amn_store_save(buckets: list, policy: dict):
    _AMN_STORE["global_policy"] = _amn_merge(_AMN_STORE["global_policy"], policy)
    bmap = {b["bucket_id"]: b for b in _AMN_STORE["buckets"]}
    for b in buckets:
        bid = b["bucket_id"]
        if bid not in bmap:
            bmap[bid] = b
...
def _amn_bucket_lookup(policy_bucket: dict, policy_global: dict) -> dict:
    m = dict(policy_global)
    for s, actions in policy_bucket.items():
        m.setdefault(s, {})
        for a, e in actions.items():
            if a not in m[s] or e["success_rate"] > m[s][a]["success_rate"]:
                m[s][a] = e
    return m
...
def _amn_seed_buckets() -> list:
    ...
    profiles = [
        {"bucket_id": "first_time_buyer",
         ...
         "markov_policy": {}},
        {"bucket_id": "investor",
         "description": "Real estate investor, cash buyer, flexible timeline",
         "markov_policy": {}},
        {"bucket_id": "relocation",
         "description": "Corporate relocation, high budget, good school district",
         "markov_policy": {}},
        {"bucket_id": "upgrade",
         "description": "Existing homeowner upgrading, family growing",
         "markov_policy": {}},
    ]
    for p in profiles:
...
def _amn_retrieve_bucket(emb: list, buckets: list) -> Optional[dict]:
    if not buckets:
        return None
    return max(buckets, key=lambda b: _amn_cosine(emb, b["centroid"]))

# ---------------------------------------------------------------------------
# AMN State Classification
# ---------------------------------------------------------------------------

_AMN_CLASSIFY_SYS = (
    "You are a real estate workflow state classifier. Return ONLY one state name:\n"
    + ", ".join(AMN_STATES) + "\n"
    "UNDERSTAND_BUYER=missing info | QUALIFY_LEAD=not qualified | SEARCH_OPTIONS=no listings | "
    "FILTER_OPTIONS=>5 listings | RANK_OPTIONS=not ranked | CONTACT_BUYER=ready to contact | "
    "SCHEDULE_TOUR=need tour | UPDATE_CRM=done, update records | DONE=complete | RECOVER=error"
)

def _amn_classify(ctx: BuyerContext) -> str:
...
_AMN_EXTRACT_SYS = (
    'Return ONLY valid JSON: {"buyer_intent":"...","budget":"...","location":"...","urgency":"high|medium|low"}\n'
    "Use empty string for unknown fields."
)

def _amn_extract(ll_ctx: LifeLoggerContext) -> BuyerContext:
    parts = [ll_ctx.conversation_summary]
    if ll_ctx.recent_speech:
        parts.append("\n".join(ll_ctx.recent_speech[-10:]))
    raw = _amn_llm(_AMN_EXTRACT_SYS, "\n\n".join(parts), max_tokens=200)
    try:
        if raw.startswith("```"):
...
def _t_extract(ctx, mode):
    cost = AMN_TOOL_COSTS["extract_requirements"] * random.uniform(0.8, 1.2)
    if not ctx.buyer_intent: ctx.buyer_intent = "Buy a home"
    return ctx, cost, True

def _t_qualify(ctx, mode):
    cost = AMN_TOOL_COSTS["qualify_lead"] * random.uniform(0.8, 1.2)
    if mode == "cold" and random.random() < 0.20: return ctx, cost, False
    ctx.qualified = True
    return ctx, cost, True

def _t_search(ctx, mode):
    cost = AMN_TOOL_COSTS["search_listings"] * random.uniform(0.8, 1.2)
    if mode == "cold" and random.random() < 0.20: return ctx, cost, False
    ctx.listings = [f"Home {i+1} at {ctx.location}" for i in range(random.randint(4, 8))]
    return ctx, cost, True

def _t_filter(ctx, mode):
    ctx.listings = ctx.listings[:3]
    return ctx, AMN_TOOL_COSTS["filter_listings"] * random.uniform(0.8, 1.2), True

def _t_rank(ctx, mode):
    ctx.ranked = True
    return ctx, AMN_TOOL_COSTS["rank_listings"] * random.uniform(0.8, 1.2), True

def _t_message(ctx, mode):
    ctx.contacted = True
    return ctx, AMN_TOOL_COSTS["send_message"] * random.uniform(0.8, 1.2), True

def _t_calendar(ctx, mode):
    return ctx, AMN_TOOL_COSTS["check_calendar"] * random.uniform(0.8, 1.2), True

def _t_tour(ctx, mode):
    cost = AMN_TOOL_COSTS["schedule_tour"] * random.uniform(0.8, 1.2)
    if not ctx.contacted: return ctx, cost, False
    ctx.tour_scheduled = True
    return ctx, cost, True

def _t_crm(ctx, mode):
    ctx.crm_updated = True
    return ctx, AMN_TOOL_COSTS["update_crm"] * random.uniform(0.8, 1.2), True

def _t_clarify(ctx, mode):
    cost = AMN_TOOL_COSTS["ask_clarifying_question"] * random.uniform(0.8, 1.2)
    if not ctx.budget:   ctx.budget = "$400,000"
    if not ctx.location: ctx.location = "Pittsburgh, PA"
    return ctx, cost, True

_AMN_TOOLS = {
    "extract_requirements": _t_extract,    "qualify_lead": _t_qualify,
    "search_listings": _t_search,          "filter_listings": _t_filter,
    "rank_listings": _t_rank,              "send_message": _t_message,
    "check_calendar": _t_calendar,         "schedule_tour": _t_tour,
    "update_crm": _t_crm,                 "ask_clarifying_question": _t_clarify,
}
...
def _amn_get_markov(state: str, policy: dict) -> tuple:
    if state not in policy or not policy[state]:
        return None, 0.0
    best, best_score = None, -1.0
    for action, e in policy[state].items():
        score = e["success_rate"] - 0.1 * e["avg_cost"]
        if score > best_score:
            best_score, best = score, action
    return best, best_score

def _amn_update_markov(policy: dict, logs: list, success: bool) -> dict:
    for log in logs:
        ...
        e = policy[s][a]
        outcome = 1.0 if (log.success and success) else (0.5 if log.success else 0.0)
        n = e["count"] + 1
        e["success_rate"] = (e["success_rate"] * e["count"] + outcome) / n
        e["avg_cost"]     = (e["avg_cost"]     * e["count"] + log.tool_cost) / n
        e["count"] = n
        e["next_state"] = log.next_state
    return policy

# ---------------------------------------------------------------------------
# AMN Action Selection
# ---------------------------------------------------------------------------

_AMN_ACTION_SYS = (
    "You are a real estate AI agent. Choose the single best next tool.\n"
...
def _amn_llm_action(state: str, ctx: BuyerContext) -> str:
    msg = (f"State:{state} intent:{ctx.buyer_intent} budget:{ctx.budget} "
           f"location:{ctx.location} qualified:{ctx.qualified} listings:{len(ctx.listings)} ranked:{ctx.ranked}")
    action = _amn_llm(_AMN_ACTION_SYS, msg, max_tokens=30).lower()
    for tool in _AMN_TOOLS:
        if tool in action:
            return tool
    return "ask_clarifying_question"

def _amn_action_valid(action: str, ctx: BuyerContext) -> bool:
    if action not in _AMN_TOOLS: return False
...
def _amn_choose(state: str, ctx: BuyerContext, policy: dict, mode: str) -> tuple:
    markov, conf = _amn_get_markov(state, policy)
    llm = _amn_llm_action(state, ctx)
    if mode == "cold":
        return llm, markov, llm, 0.0
    if conf > AMN_CONFIDENCE_THRESHOLD and markov and _amn_action_valid(markov, ctx):
        return llm, markov, markov, conf
    return llm, markov, llm, conf

# ---------------------------------------------------------------------------
# AMN Synthetic Bootstrap
# ---------------------------------------------------------------------------
...
def _amn_synth_step(num, state, action, next_state, success):
    return StepLog(num, state, action, None, action, 0.0, next_state,
                   AMN_TOOL_COSTS.get(action, 0.02) * random.uniform(0.8, 1.2), success)

def _amn_gen_trace(profile: dict, seed: int = 0) -> list:
    random.seed(seed)
    path, logs, step, i = profile["ideal_path"], [], 1, 0
    while i < len(path) and step <= 20:
        state, action = path[i]
        nxt = path[i+1][0] if i+1 < len(path) else "DONE"
        if random.random() < 0.15 and step > 1:
            alts = _AMN_ALTS.get(state, [action])
            wrong = random.choice([a for a in alts if a != action] or [action])
            if random.random() < 0.6:
                logs.append(_amn_synth_step(step, state, wrong, "RECOVER", False)); step += 1
                logs.append(_amn_synth_step(step, "RECOVER", "ask_clarifying_question", state, True)); step += 1
                continue
        ok = random.random() > 0.05
        logs.append(_amn_synth_step(step, state, action, nxt if ok else "RECOVER", ok)); step += 1
        if not ok:
            logs.append(_amn_synth_step(step, "RECOVER", "ask_clarifying_question", state, True)); step += 1
        else:
            i += 1
    if i >= len(path):
        logs.append(_amn_synth_step(step, "UPDATE_CRM", "update_crm", "DONE", True))
    return logs

def _amn_bootstrap():
    """Generate 40 synthetic traces and load into in-memory store."""
    traces = [_amn_gen_trace(p, seed=hash(p["profile"]) + i)
...
def _amn_run_agent(ctx: BuyerContext, policy: dict, mode: str, buckets: list) -> tuple:
    ...
    while state != "DONE" and len(logs) < AMN_MAX_STEPS:
        ...
        ctx, cost, success = _AMN_TOOLS[chosen](ctx, mode) if chosen in _AMN_TOOLS else (ctx, 0.01, False)
        next_state = "RECOVER" if not success else _amn_classify(ctx)
        logs.append(StepLog(len(logs)+1, state, llm_act, markov_act, chosen, conf, next_state, cost, success))
        total_cost += cost
        state = next_state
        time.sleep(0.2)
    return logs, total_cost, (state == "DONE")

def _amn_run_warm(ll_ctx: LifeLoggerContext) -> tuple:
    """
    Thread-safe entry point called from trigger_amn().
    Bootstraps if needed, extracts task, runs warm agent, saves learnings.
    """
    buckets, policy = _amn_store_load()
    if not buckets:
        _amn_bootstrap()
        buckets, policy = _amn_store_load()

    ctx = _amn_extract(ll_ctx)
    logs, cost, success = _amn_run_agent(ctx, policy, "warm", buckets)
    updated = _amn_update_markov(dict(policy), logs, success)
    _amn_store_save(buckets, updated)
    return logs, cost, success
